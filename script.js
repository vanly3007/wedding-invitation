...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //     "Chùc m?ng h?nh phùc! ??",
    //     "Mùi yùu nhù! ?",
    //     "Tr?m n?m h?nh phùc nha",
    //     "Tuy?t v?i quù ?i!",
    //     "Ch? ngùy nùy lùu l?m r?i",
    //     "H?i ??ng & Minh Nguy?t",
    //     "H?nh phùc nha!",
    //     "Quù lù x?ng ?ùi luùn",
    //     "G?i ngùn l?i chùc t?t ??p",
    //     "Tùnh yùu v?nh c?u"
    // ];
...
function submitRSVP() {
    ...
    if(!name) {
        alert('Vui lùng nh?p tùn c?a b?n!');
        return;
    }
...
function sendWish() {
    ...
    alert('C?m ?n b?n ?ù g?i l?i chùc: ' + input.value);
    input.value = '';
}

function showGiftModal() {
    alert('Thùng tin chuy?n kho?n: \n- Ngùn hùng: VCB \n- STK: 1234567890 \n- Ch? TK: NGUYEN HAI DANG');
}
...
function copyToClipboard(text) {
  navigator.clipboard.writeText(text).then(() => {
    alert('ùù sao chùp s? tùi kho?n: ' + text);
  }).catch(err => {
    console.error('L?i khi sao chùp: ', err);
  });
}
